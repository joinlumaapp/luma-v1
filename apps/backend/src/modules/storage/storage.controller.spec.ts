...
describe('StorageController', () => {
  ...
  describe('uploadFile()', () => {
    ...
    it('should throw BadRequestException when file exceeds max size', async () => {
      ...
      await expect(
        controller.uploadFile(userId, file),
  ...
  describe('uploadProfilePhoto()', () => {
    ...
    it('should upload a profile photo successfully', async () => {
      ...
      const result = await controller.uploadProfilePhoto(userId, validFile, '2');
  ...
  describe('uploadChatImage()', () => {
    ...
    it('should throw BadRequestException when file exceeds max size', async () => {
      ...
      await expect(
        controller.uploadChatImage(userId, bigFile),
  ...
  describe('deleteFile()', () => {
    it('should delete a file successfully', async () => {
      ...
      const result = await controller.deleteFile('user-1', 'photos/user-1/abc.jpg');

      expect(result).toEqual({ deleted: true });
      expect(mockStorageService.deleteFile).toHaveBeenCalledWith('photos/user-1/abc.jpg');
    });

    it('should throw BadRequestException when no key provided', async () => {
      await expect(
        controller.deleteFile('user-1', ''),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe('getSignedUrl()', () => {
    it('should return a signed URL with default expiry', async () => {
      mockStorageService.getSignedUrl.mockResolvedValue('https://signed.example.com');

      const result = await controller.getSignedUrl('user-1', 'photos/user-1/abc.jpg');

      expect(result.url).toBe('https://signed.example.com');
      expect(result.expiresIn).toBe(3600);
      expect(mockStorageService.getSignedUrl).toHaveBeenCalledWith('photos/user-1/abc.jpg', 3600);
    });

    it('should use custom expiresIn when provided', async () => {
      mockStorageService.getSignedUrl.mockResolvedValue('https://signed.example.com');

      const result = await controller.getSignedUrl('user-1', 'photos/user-1/abc.jpg', '600');

      expect(result.expiresIn).toBe(600);
      expect(mockStorageService.getSignedUrl).toHaveBeenCalledWith('photos/user-1/abc.jpg', 600);
    });

    it('should throw BadRequestException when no key provided', async () => {
      await expect(
        controller.getSignedUrl('user-1', ''),
      ).rejects.toThrow(BadRequestException);
    });

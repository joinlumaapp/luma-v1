...
describe('StorageService', () => {
  ...
  const mockImageProcessor = {
    processProfilePhoto: jest.fn().mockResolvedValue({ buffer: Buffer.from('processed'), size: 100 }),
    generateThumbnail: jest.fn().mockResolvedValue({ buffer: Buffer.from('thumb'), size: 50 }),
  };
  ...
  describe('uploadFile()', () => {
    ...
    it('should throw BadRequestException for empty file', async () => {
      await expect(
        service.uploadFile(Buffer.alloc(0), 'docs'),
    ...
    it('should propagate S3 errors', async () => {
      ...
      await expect(
        service.uploadFile(Buffer.from('data'), 'test', { contentType: 'image/jpeg' }),
  ...
  describe('uploadProfilePhoto()', () => {
    it('should process photo, generate thumbnail, and upload both to S3', async () => {
      ...
      expect(result.thumbnailUrl).toContain('thumbnails/user-1/test-uuid-1234.jpg');
      expect(mockSend).toHaveBeenCalledTimes(2); // photo + thumbnail
      expect(mockImageProcessor.processProfilePhoto).toHaveBeenCalledWith(buffer);
    ...
    it('should use CloudFront URLs when configured', async () => {
      ...
      const result = await cfService.uploadProfilePhoto('user-1', Buffer.from('data'), 0);

      expect(result.url).toBe('https://cdn.luma.app/photos/user-1/test-uuid-1234.jpg');
      expect(result.thumbnailUrl).toBe('https://cdn.luma.app/thumbnails/user-1/test-uuid-1234.jpg');
    });
  });
  ...
  describe('uploadPhoto()', () => {
    ...
    it('should upload a photo and return url, thumbnailUrl, and key', async () => {
      ...
      expect(result.thumbnailUrl).toContain('thumbnails/user-123/test-uuid-1234.jpg');
      expect(mockSend).toHaveBeenCalledTimes(1);
    });
    ...
    it('should use CloudFront URL when configured', async () => {
      ...
      expect(result.thumbnailUrl).toBe('https://cdn.luma.app/thumbnails/user-123/test-uuid-1234.jpg');
    });
    ...
    it('should default to jpg extension for unknown MIME types', async () => {
      ...
      const result = await service.uploadPhoto(userId, jpegBuffer, 'image/jpeg');
  ...
  describe('uploadProfileVideo()', () => {
    ...
    it('should upload a video and return url, thumbnailUrl, key, duration', async () => {
      const result = await service.uploadProfileVideo(userId, videoBuffer, 'video/mp4', 10);

      expect(result.key).toBe('videos/user-789/test-uuid-1234.mp4');
      expect(result.url).toContain('videos/user-789/test-uuid-1234.mp4');
      expect(result.thumbnailUrl).toContain('thumbnails/user-789/test-uuid-1234_video.jpg');
      expect(result.duration).toBe(10);
      expect(mockSend).toHaveBeenCalledTimes(1);
    });

    it('should use mov extension for video/quicktime', async () => {
      const result = await service.uploadProfileVideo(userId, videoBuffer, 'video/quicktime', 5);

      expect(result.key).toBe('videos/user-789/test-uuid-1234.mov');
    });

    it('should default duration to 0 when not provided', async () => {
      const result = await service.uploadProfileVideo(userId, videoBuffer, 'video/mp4');
  ...
  describe('initialization', () => {
    it('should initialize in S3 mode when credentials are present', async () => {
      ...
      const result = await svc.uploadPhoto('u1', Buffer.from('data'), 'image/jpeg');

...
describe('CacheKeys', () => {
  ...
  describe('discoveryFeed()', () => {
    ...
    it('should build key with filters hash', () => {
      expect(CacheKeys.discoveryFeed('u1', 'abc123')).toBe('discovery:feed:u1:abc123');
    });

    it('should handle undefined filters hash', () => {
      expect(CacheKeys.discoveryFeed('u1', undefined)).toBe('discovery:feed:u1');
    });
  });
  ...
  describe('compatibilityScore()', () => {
    it('should order user IDs alphabetically', () => {
      expect(CacheKeys.compatibilityScore('b-user', 'a-user')).toBe('compat:score:a-user:b-user');
    });

    it('should keep order when already sorted', () => {
      expect(CacheKeys.compatibilityScore('a-user', 'b-user')).toBe('compat:score:a-user:b-user');
    });

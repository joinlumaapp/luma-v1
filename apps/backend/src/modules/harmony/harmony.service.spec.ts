...
describe('HarmonyService', () => {
  ...
  describe('getUserSessions()', () => {
    ...
    it('should map game cards correctly', async () => {
      ...
      expect((result.sessions[0].cards[0] as { gameType: string }).gameType).toBe('WOULD_YOU_RATHER');
    });
  ...
  describe('createSession()', () => {
    ...
    const setupSuccessfulCreate = (overrides: { userId?: string; deckCategory?: string } = {}) => {
    ...
    it('should use deckCategory filter when provided', async () => {
      ...
      await service.createSession('u1', { matchId: 'm1', deckCategory: 'ICEBREAKER' });
  ...
  describe('getSession()', () => {
    ...
    it('should auto-end expired sessions and check badges', async () => {
      ...
      expect(mockBadgesService.checkAndAwardBadges).toHaveBeenCalledWith('u1', 'harmony');
      expect(mockBadgesService.checkAndAwardBadges).toHaveBeenCalledWith('u2', 'harmony');
    });

    it('should handle badge check failure gracefully', async () => {
      mockBadgesService.checkAndAwardBadges.mockRejectedValue(new Error('Badge error'));
      mockPrisma.harmonySession.findUnique.mockResolvedValue({
        id: 's1',
        userAId: 'u1',
        userBId: 'u2',
    ...
    it('should map question and game cards in getSessionCards', async () => {
      ...
      mockPrisma.harmonyUsedCard.findMany.mockResolvedValue([
        {
          usedAt: new Date(),
          questionCard: { id: 'qc1', category: 'DEEP_CONNECTION', textEn: 'Q?', textTr: 'S?' },
          gameCard: null,
        },
        {
          usedAt: new Date(),
  ...
  describe('getCards()', () => {
    it('should throw NotFoundException when session not found', async () => {
      ...
      await expect(service.getCards('u1', 's-bad')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ForbiddenException when user is not participant', async () => {
      ...
      await expect(service.getCards('u1', 's1')).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should return cards for valid participant', async () => {
      ...
      mockPrisma.harmonyUsedCard.findMany.mockResolvedValue([
        {
          usedAt: new Date(),
          questionCard: { id: 'qc1', category: 'ICEBREAKER', textEn: 'Q?', textTr: 'S?' },
          gameCard: null,
        },
      ]);

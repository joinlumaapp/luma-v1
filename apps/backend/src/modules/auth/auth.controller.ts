...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      "Creates a new user account (if phone is new) and sends a 6-digit SMS verification code. For existing users, sends a fresh OTP for login.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Verifies the 6-digit OTP code. On success, marks phone as verified and returns JWT access + refresh tokens.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Compares selfie against profile photos for identity verification. Awards verified badge on success.",
  })
  @ApiResponse({ status: 200, description: "Verification result returned" })
  @ApiResponse({
    status: 400,
    description: "Selfie too large or user not found",
  })
  async verifySelfie(
  ...
  @ApiOperation({
    ...
    description:
      "Verifies OTP for an existing user and returns JWT access + refresh tokens. Call /register first to receive the OTP.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Anonymizes personal data, cancels subscriptions, removes from discovery pool. Data permanently deleted after 30 days.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Returns all personal data associated with the account in JSON format per GDPR Article 20.",
  })

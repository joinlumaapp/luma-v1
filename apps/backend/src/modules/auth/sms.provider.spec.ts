...
describe("SmsProvider", () => {
  ...
  describe("production mode — Turkish numbers (+90)", () => {
    ...
    it("should throw when all providers fail", async () => {
      ...
      await expect(
        smsProvider.sendOtp("+905551234567", "123456"),
      ).rejects.toThrow("SMS gonderilemedi");
    });
  });
  ...
  describe("rate limiting", () => {
    ...
    it("should reject when rate limit exceeded (3 requests in 10 min)", async () => {
      ...
      await expect(
        smsProvider.sendOtp("+905551234567", "123456"),
      ).rejects.toThrow("SMS gonderim limiti asildi");
    });
  ...
  describe("OTP storage and verification", () => {
    ...
    it("should throw on wrong OTP code", async () => {
      ...
      await expect(
        smsProvider.verifyOtp("+905551234567", "999999"),
      ).rejects.toThrow("Gecersiz dogrulama kodu");
    ...
    it("should throw when OTP not found (expired from Redis)", async () => {
      ...
      await expect(
        smsProvider.verifyOtp("+905551234567", "123456"),
      ).rejects.toThrow("Dogrulama kodu bulunamadi");
    });
    ...
    it("should throw when max attempts exceeded", async () => {
      ...
      await expect(
        smsProvider.verifyOtp("+905551234567", "123456"),
      ).rejects.toThrow("Cok fazla hatali deneme");
    ...
    it("should track remaining attempts correctly", async () => {
      ...
      await expect(
        smsProvider.verifyOtp("+905551234567", "999999"),
      ).rejects.toThrow("1 deneme hakkiniz kaldi");
    });
  });
});
